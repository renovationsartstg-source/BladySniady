...
# --- KONFIGURACJA STRONY ---
st.set_page_config(
    page_title="Bladysniady | Admin Panel",
    page_icon="🎮",
    layout="wide",
)

# --- PANEL ADMINISTRATORA (SIDEBAR) ---
st.sidebar.title("🛠️ Panel Admina")
password = st.sidebar.text_input("Hasło", type="password")

# Inicjalizacja domyślnych statystyk w pamięci sesji
if 'followers' not in st.session_state:
    st.session_state.followers = "250K+"
if 'wins' not in st.session_state:
    st.session_state.wins = "1,200+"
if 'hours' not in st.session_state:
    st.session_state.hours = "5,000+"
if 'live_status' not in st.session_state:
    st.session_state.live_status = "LIVE NOW"

# Sprawdzenie hasła
if password == "admin123":
    st.sidebar.success("Zalogowano!")
    st.session_state.followers = st.sidebar.text_input("Obserwujący", st.session_state.followers)
    st.session_state.wins = st.sidebar.text_input("Wygrane", st.session_state.wins)
    st.session_state.hours = st.sidebar.text_input("Godziny", st.session_state.hours)
    st.session_state.live_status = st.sidebar.selectbox("Status Live", ["LIVE NOW", "OFFLINE"])
else:
    if password != "":
        st.sidebar.error("Błędne hasło")
    st.sidebar.info("Wpisz hasło 'admin123', aby edytować stronę.")
...
# --- DYNAMICZNY KOD HTML ---
# Wstawiamy zmienne z Pythona {st.session_state...} bezpośrednio do HTML
html_code = f"""
<!DOCTYPE html>
<html lang="pl">
<head>
    ...
    <style>
        * {{ margin:0; padding:0; box-sizing:border-box; scroll-behavior:smooth; }}
        ...
        .live-badge {{ 
            ...
            border:2px solid {'red' if st.session_state.live_status == 'LIVE NOW' else '#444'}; 
            border-radius:20px; 
            box-shadow:0 0 20px {'red' if st.session_state.live_status == 'LIVE NOW' else 'transparent'}; 
            margin-top:20px; 
        }}
        .dot {{ width:10px; height:10px; background:{'red' if st.session_state.live_status == 'LIVE NOW' else '#444'}; border-radius:50%; animation:blink 1s infinite; }}
...
<body>
    ...
    <section id="stats" class="reveal">
        <h3>PLAYER STATS</h3>
        <div class="stats">
            <div class="stat"><h4>{st.session_state.followers}</h4><p>Followers</p></div>
            <div class="stat"><h4>{st.session_state.wins}</h4><p>Wins</p></div>
            <div class="stat"><h4>{st.session_state.hours}</h4><p>Hours Streamed</p></div>
        </div>
    </section>
